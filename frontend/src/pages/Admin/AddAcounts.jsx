...
function AddAcounts() {
    ...
    const fetchAllTables = async () => {
        const [callcenterResponse, sysadminResponse, chefEquipeResponse] = await Promise.all([
            axiosClient.get('api/callcenter'),
            axiosClient.get('api/sysadmin'),
            axiosClient.get('api/chefequipe')
        ]);
        return {
            callcenter: callcenterResponse.data,
            sysadmin: sysadminResponse.data,
            chef_equipe: chefEquipeResponse.data
        };
    };

    const { data, error, isLoading, refetch } = useQuery('allTables', fetchAllTables);
    const { callcenter = [], sysadmin = [], chef_equipe = [] } = data || {};

    // Use useMemo to prevent unnecessary recalculations
    const combinedData = useMemo(() => [...callcenter, ...sysadmin, ...chef_equipe], [callcenter, sysadmin, chef_equipe]);
        ...
        <>
            ...
            <Box sx={{ width: '100%', typography: 'body1' }}>
                <TabContext value={value}>
                                            ...
                                            {filteredRows.length > 0 ? (
                                                filteredRows.map((item) => (
                                                    <TableRow key={item.email}>
                                                        ...
                                                        <TableCell>{item.CIN === undefined ? ('...............') : (item.CIN)}</TableCell>
                                                        <TableCell>{item.telephone === undefined ? ('...............') : (item.telephone)}</TableCell>
                    ...
                    <TabPanel value="2">
                        <Register tab={setValue} refetch={refetch} />
                    </TabPanel>
                </TabContext>
            </Box>
        </>
